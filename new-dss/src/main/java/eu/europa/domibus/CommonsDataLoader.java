...
public class CommonsDataLoader implements DataLoader {
	...
	private HttpClientBuilder configureProxy(HttpClientBuilder httpClientBuilder,
		...
		final boolean proxyHTTPS = Protocol.isHttps(protocol) && (httpsConfig!=null);
		final boolean proxyHTTP = Protocol.isHttp(protocol) && (httpsConfig!=null);
		...
		int proxyPort = Integer.parseInt(props.getProperty("port"));
		String proxyUser = props.getProperty("user");
		String proxyPassword = props.getProperty("password");
		String proxyExcludedHosts = props.getProperty("excludedHosts");
