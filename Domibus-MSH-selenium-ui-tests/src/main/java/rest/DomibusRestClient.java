...
public class DomibusRestClient {
    ...
    public void switchDomain(String domainName) {
        ...
        if (getDomainCodes().contains(domainName)) {
            ...
            builder.accept(MediaType.TEXT_PLAIN_TYPE).type(MediaType.TEXT_PLAIN_TYPE).put(ClientResponse.class, domainName);
        }
    ...
    public JSONArray getListOfMessages(String domain) throws Exception {
        ...
        ClientResponse clientResponse = requestGET(resource.path(RestServicePaths.MESSAGE_LOG_MESSAGES), params);
        if (clientResponse.getStatus() != 200) {
    ...
    public JSONArray getAllAlerts(String domain,String domainAlert) throws Exception {
        switchDomain(domain);
