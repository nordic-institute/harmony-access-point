...
//		Delete created filter
    ...
    public void changeMsgFilterOnUpdate() throws Exception {
        ...
        if (popup.getPluginSelect().getOptionsTexts().size() > 1) {

            log.info("Select plugin at index 1");
            popup.getPluginSelect().selectOptionByIndex(1);
            log.info("Enter action field data");
            popup.getActionInput().fill(Generator.randomAlphaNumeric(3));
            ...
            page.refreshPage();
            log.info(" only 1 filter is available in installation, so unable to change plugin filter from edit pop up");
    ...
    public void exportCsvOnUnsavedFilterDelAction() throws Exception {
        ...
        int csvRowCount = DFileUtils.getRowCount(completeFilePath) - 1;
        ...
        int indexx = page.grid().scrollTo("Action", actionName);
        if (indexx < 0) {
            throw new RuntimeException("Could not find created filter");
        }
        ...
        log.info("Customized location for download");
        String filePathh = DFileUtils.downloadFolderPath();


        log.info("Clean given directory");
        FileUtils.cleanDirectory(new File(filePathh));

        log.info("Click on download csv button");
        page.clickDownloadCsvButton(page.getDownloadCsvButton().element);

        new Dialog(driver).cancel();
        String completeFilePathh = filePath + File.separator + DFileUtils.getCompleteFileName(filePathh);

        int csvRowCountt = DFileUtils.getRowCount(completeFilePathh) - 1;
        int gridRowCountt = page.grid().getPagination().getTotalItems();


        soft.assertTrue(csvRowCountt - 1 == gridRowCountt, "Newly added record should be present in downloaded csv only");
        soft.assertTrue(page.getSaveBtn().isEnabled(), "Save button is still enabled");
        soft.assertTrue(page.getCancelBtn().isEnabled(), "Cancel button is stil enabled");
        soft.assertAll();
    ...
    public void editWithDiffRowSelection() throws Exception {
        ...
        String actionName = Generator.randomAlphaNumeric(5);
        String actionNamee = Generator.randomAlphaNumeric(5);
        String newActionName = Generator.randomAlphaNumeric(5);

        log.info("create new message filter using rest call");
        rest.createMessageFilter(actionName, null);
        rest.createMessageFilter(actionNamee, null);

        page.refreshPage();

        log.info("Identify row number for created filters");
        int index = page.grid().scrollTo("Action", actionName);
        int indexx = page.grid().scrollTo("Action", actionNamee);

        log.info("Select row for first filter");
        page.grid().selectRow(index);

        log.info("Click on edit icon for second filter");
        page.rowWiseActionButton(indexx, "edit").click();
        MessageFilterModal modal = new MessageFilterModal(driver);

        log.info("Verify edit pop up field values as per second filter values");
        soft.assertTrue(modal.getActionInput().getText().equalsIgnoreCase(actionNamee));
        log.info("editing action value");
        modal.getActionInput().fill(newActionName);
        modal.clickOK();
        ...
        page.saveAndConfirmChanges();
        soft.assertTrue(page.grid().scrollTo("Action", newActionName) == indexx);
        soft.assertAll();
    ...
    public void delWithDiffRowSelection() throws Exception {
        ...
        page.rowWiseActionButton(indexx, "delete").click();
        ...
        soft.assertTrue(page.grid().scrollTo("Action", actionNamee) < 0);
    ...
    public void moveUpWithDiffRowSelection() throws Exception {
        ...
        log.info("Click on moveup icon for secodn row");
        page.rowWiseActionButton(indexx, "moveup").click();

        log.info("Check status for moveup and move down button for selected row");
        ...
        soft.assertTrue(page.grid().scrollTo("Action", actionNamee) == indexx - 1);
        soft.assertAll();
    }
    ...
    public void moveDownWithDiffRowSelection() throws Exception {
        ...
        log.info("Create message filter using rest call");
        ...
        log.info("Identify row number for both filters");

        int index = page.grid().scrollTo("Action", actionName);
        int indexx = page.grid().scrollTo("Action", actionNamee);

        log.info("Select row for second filter");
        page.grid().selectRow(indexx);

        log.info("Click on movedown for first filter");
        page.rowWiseActionButton(index, "movedown").click();

        soft.assertTrue(page.getSaveBtn().isEnabled());
        log.info("Check status for moveup and movedown for selected row");
        if (page.grid().scrollTo("Action", actionName) + 1 == page.grid().getPagination().getTotalItems()) {

            soft.assertTrue(page.getMoveUpBtn().isEnabled());

        } else {
            soft.assertTrue(page.getMoveUpBtn().isEnabled() && page.getMoveDownBtn().isEnabled());

        }
        log.info("Clcik on save then ok ");
        page.saveAndConfirmChanges();
        page.waitForTitle();

        log.info("check row number for both filters after move down operation");
        soft.assertTrue(page.grid().scrollTo("Action", actionName) == index + 1);
        soft.assertTrue(page.grid().scrollTo("Action", actionNamee) == indexx - 1);
        soft.assertAll();
    }
