...
public class JMSMessPgTest extends SeleniumTest {
	...
	/*  JMS-7 - Delete message  */
	@Description("JMS-7 - Delete message")
	@Link(name = "EDELIVERY-5134", url = "https://ec.europa.eu/cefdigital/tracker/browse/EDELIVERY-5134")
	@AllureId("JMS-7")
	...
	public void deleteJMSMessage() throws Exception {
		...
		if (StringUtils.isEmpty(qWMess)) {
			...
			Allure.step("deleting first message listed");
			...
			page.grid().waitForRowsToLoad();
			Allure.step("check message is deleted from grid");
	...
	/*JMS-8 - Move message*/
	/*  JMS-8 - Move message  */
	@Description("JMS-8 - Move message")
	@Link(name = "EDELIVERY-5135", url = "https://ec.europa.eu/cefdigital/tracker/browse/EDELIVERY-5135")
	@AllureId("JMS-8")
	...
	public void moveMessage() throws Exception {
		...
		if (StringUtils.isEmpty(qWMess)) {
			...
			Allure.step("moving the first message");
			...
			Allure.step("checking the number of messages");
			...
			Allure.step("getting no of messages in DLQ queue");
			...
			Allure.step("selecting queue " + queuename);
			...
			HashMap<String, String> rowInfo = page.grid().getRowInfo(0);
			page.grid().selectRow(0);
			Allure.step("moving message on row 0 to DLQ queue");
			...
			Allure.step("checking number of listed messages for this queue");
			...
			Allure.step("selecting DLQ queue");
			log.info("selecting DLQ queue");
			page.filters().getJmsQueueSelect().selectDLQQueue();
			page.grid().waitForRowsToLoad();

			Allure.step("checking no of messages in DLQ queue");
			...
			Allure.step("checking the moved message is present in the grid");
	...
	/*  JMS-9 - Domain admin logs in and views messages  */
	@Description("JMS-9 - Domain admin logs in and views messages")
	@Link(name = "EDELIVERY-5136", url = "https://ec.europa.eu/cefdigital/tracker/browse/EDELIVERY-5136")
	@AllureId("JMS-9")
	...
	public void adminOpenJMSMessagesPage() throws Exception {
		...
		Allure.step("checking for domain " + domainCode);
		...
		Allure.step("logging in with admin " + user.getString("userName"));
		log.info("logging in with admin " + user.getString("userName"));

		JMSMonitoringPage page = new JMSMonitoringPage(driver);
		page.getSidebar().goToPage(PAGES.JMS_MONITORING);

		Allure.step("checking domain name in the title");
		...
		Allure.step("checking message numbers are missing from queue source names");
		...
		Allure.step("checking messages contain domain name in Custom prop field");
...
//		destination = queues.getJSONObject(i).getString("name");
	...
	@Description("JMS-10 - Super admin logs in and views messages for a selected domain, selects 1 message, and changes domain")
	@Link(name = "EDELIVERY-5137", url = "https://ec.europa.eu/cefdigital/tracker/browse/EDELIVERY-5137")
	@AllureId("JMS-10")
	...
	public void changeDomainAfterSelection() throws Exception {
		...
		JMSMonitoringPage page = new JMSMonitoringPage(driver);
		Allure.step("Login into application and navigate to JMS Monitoring page");
		log.info("Login into application and navigate to JMS Monitoring page");
		page.getSidebar().goToPage(PAGES.JMS_MONITORING);
		page.grid().waitForRowsToLoad();

		Allure.step("Choose domain name from page title");
		...
		Allure.step("select any message queue having some messages");
		...
		Allure.step("select first row");
		log.info("select first row");
		page.grid().selectRow(0);

		Allure.step("Confirm status of Move button and Delete button");
		...
		Allure.step("Compare old and new domain name");
		...
		Allure.step("Check status of move button and delete button");
	...
	@Description("JMS-11 - Super admin logs in and views messages for a selected domain, navigates to second page of messages and changes domain")
	@Link(name = "EDELIVERY-5138", url = "https://ec.europa.eu/cefdigital/tracker/browse/EDELIVERY-5138")
	@AllureId("JMS-11")
...
//		page.filters().getJmsQueueSelect().selectOptionByText(q);
		...
		Allure.step("going to next page");
	...
	@Description("JMS-12 - Super admin selects a message and chooses to delete it")
	@Link(name = "EDELIVERY-5139", url = "https://ec.europa.eu/cefdigital/tracker/browse/EDELIVERY-5139")
	@AllureId("JMS-12")
	...
	public void jmsMsgDelOnDomainChange() throws Exception {
		...
		Allure.step("Verify status of Move button and Delete button");
		...
		Allure.step("Check message count in queue");
		log.info("Check message count in queue");
		page.grid().waitForRowsToLoad();

		page.filters().getJmsQueueSelect().selectQueueByName(q);

		Allure.step("After domain chenge select any message queue having some messages");
		...
		Allure.step("Current message count is " + totalCount);
		...
		Allure.step("Check presence of success message on deletion");
		...
		Allure.step("Verify queue message count as 1 less than before");
	...
	@Description("JMS-21 - Max date for Received Up To field   ")
	@Link(name = "EDELIVERY-5148", url = "https://ec.europa.eu/cefdigital/tracker/browse/EDELIVERY-5148")
	@AllureId("JMS-21")
	...
	public void checkReceivedUpTo() throws Exception {
		...
		Allure.step("getting expectyed date");
	...
	@Description("JMS-23 - Check queue message  count against each JMS queue in Search filter Source field in case of Admin")
	@Link(name = "EDELIVERY-5150", url = "https://ec.europa.eu/cefdigital/tracker/browse/EDELIVERY-5150")
	@AllureId("JMS-23")
	@Test(description = "JMS-23", groups = {"multiTenancy", "singleTenancy"})
	public void queueMsgCountOnInputFilter() throws Exception {
		SoftAssert soft = new SoftAssert();

		Allure.step("navigate to JMS Monitoring page");
		...
		if (data.isMultiDomain()) {
			...
			Allure.step("login with admin and navigate to JMS Monitoring page");
	...
	@Description("JMS-24 - Check queue message count against each queue in destination on Move pop up in case of admin ")
	@Link(name = "EDELIVERY-5151", url = "https://ec.europa.eu/cefdigital/tracker/browse/EDELIVERY-5151")
	@AllureId("JMS-24")
	...
	public void queueMsgCountOnMovePopUp() throws Exception {
		...
		Allure.step("selecting queue with name " + q);
		...
		if (data.isMultiDomain()) {
			Allure.step("Create Admin user for default domain");
			...
			Allure.step("Login into application with domain admin and navigate to JMS Monitoring page");
	...
	@Description("JMS-25 - Check queue message  count against each JMS queue in Search filter Source field in case of Super  Admin")
	@Link(name = "EDELIVERY-5152", url = "https://ec.europa.eu/cefdigital/tracker/browse/EDELIVERY-5152")
	@AllureId("JMS-25")
	...
	@Description("JMS-26 - Check queue message count against each queue in destination on Move pop up in case of Super admin")
	@Link(name = "EDELIVERY-5153", url = "https://ec.europa.eu/cefdigital/tracker/browse/EDELIVERY-5153")
	@AllureId("JMS-26")
	...
	private void verifyQueueHasMessageCount(List<String> queues, Boolean expectCount, SoftAssert soft) {
		Allure.step("Checking message count in queue names is present");
		log.info("Checking message count in queue names is present");
		Allure.step("Expecting count = " + expectCount);
