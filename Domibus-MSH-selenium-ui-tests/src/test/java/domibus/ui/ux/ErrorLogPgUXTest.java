...
public class ErrorLogPgUXTest extends BaseUXTest {
	...
	public void filterUsingAdvancedFilters() throws Exception {
		...
		log.info("filtering by data listed in first row of grid: " + row);
		page.filters().advancedSearch(row.get("Signal Message Id"), row.get("Message Id"), row.get("Timestamp"), null,
				row.get("Error Detail"), row.get("AP Role"), row.get("Error Code"), null, null);
		grid.waitForRowsToLoad();
