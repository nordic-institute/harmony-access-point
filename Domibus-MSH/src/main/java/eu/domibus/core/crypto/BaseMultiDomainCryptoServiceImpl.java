 ...
 * @since 5.0
 */
public abstract class BaseMultiDomainCryptoServiceImpl implements MultiDomainCryptoService {

    private static final DomibusLogger LOG = DomibusLoggerFactory.getLogger(BaseMultiDomainCryptoServiceImpl.class);
        ...
        if (domainCertificateProviderMap.get(domain) == null) {
            synchronized (domainCertificateProviderMap) {
                if (domainCertificateProviderMap.get(domain) == null) { //NOSONAR: double-check locking
                    ...
                    DomainCryptoService domainCertificateProvider = createForDomain(domain);
