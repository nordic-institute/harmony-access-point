...
@NamedQueries({
        ...
        @NamedQuery(name = "MessageGroupEntity.findSendNonExpiredOrRejected", query = "SELECT c FROM MessageGroupEntity c, UserMessageLog msg where c.mshRole = :MSH_ROLE " +
                " and ( (c.rejected is null or c.rejected=false) or (c.expired is null or c.expired=false) ) and c.sourceMessageId = msg.messageId and msg.messageStatus = :SOURCE_MSG_STATUS")
})
