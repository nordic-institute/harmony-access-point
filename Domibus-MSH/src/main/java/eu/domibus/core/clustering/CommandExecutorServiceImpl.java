...
public class CommandExecutorServiceImpl implements CommandExecutorService {
    ...
    public void executeCommands(String serverName, Domain domain) {
        ...
        for (Command command : commandsByServerName) {
            //submits a task so that the @Transactional annotation is taken into account(method is in the same Spring bean)
            domainTaskExecutor.submit(() -> executeAndDeleteCommand(command, domain), domain);
        }
    }
