...
import eu.domibus.plugin.fs.custom.Domain4Entity;
...
public class Domain4Worker extends DomibusQuartzJobExtBean {

    private static final DomibusLogger LOG = DomibusLoggerFactory.getLogger(Domain4Worker.class);
    ...
    protected void executeJob(JobExecutionContext context, DomainDTO domain) {
        LOG.info("--------------Executing Domain4 job");

        if (!domain.getCode().equalsIgnoreCase("domibus_domain_4")) {
            LOG.debug("Job is not allowed to run on domain [{}]", domain.getCode());
            return;
        }

        Thread.currentThread().setContextClassLoader(this.getClass().getClassLoader());
        List<Domain4Entity> allFSPluginEntities = fsPluginEntityDao.findAllForDomain4();

        LOG.info("--------------Finished executing Domain4 job found [{}]", allFSPluginEntities.size());
    }
