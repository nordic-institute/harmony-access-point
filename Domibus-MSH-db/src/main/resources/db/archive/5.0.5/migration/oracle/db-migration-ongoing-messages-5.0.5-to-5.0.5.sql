-- ********************************************************************************************************
-- Domibus 5.0.5 to 5.0.5 ongoing messages data migration package
--
...
CREATE OR REPLACE PACKAGE MIGRATE_ONGOING_MESSAGES_505 IS
...
END MIGRATE_ONGOING_MESSAGES_505;
/

CREATE OR REPLACE PACKAGE BODY MIGRATE_ONGOING_MESSAGES_505 IS
        ...
        CURSOR c_user_message IS
            ...
            WHERE ID_PK IN (
                ...
                WHERE MESSAGE_STATUS_ID_FK IN (
                    SELECT ID_PK
                    FROM TB_D_MESSAGE_STATUS
                    WHERE STATUS IN ('SEND_ENQUEUED', 'WAITING_FOR_RETRY', 'READY_TO_PULL', 'WAITING_FOR_RECEIPT', 'SEND_FAILURE')
    ...
    PROCEDURE migrate_ws_plg_backend_msg_log(db_link IN VARCHAR2, migration IN T_MIGRATION_DETAILS) IS
        CURSOR c_ws_backebd_plugin_msg_log IS
            ...
            WHERE MESSAGE_STATUS IN ('SEND_ENQUEUED', 'WAITING_FOR_RETRY', 'READY_TO_PULL', 'WAITING_FOR_RECEIPT', 'SEND_FAILURE')
              AND MESSAGE_ID IN (
                SELECT MESSAGE_ID
                FROM WS_PLUGIN_TB_MESSAGE_LOG
                WHERE RECEIVED BETWEEN migration.startDate AND migration.endDate);
    ...
    BEGIN
        ...
        LOOP
            ...
            FOR i IN ws_plugin_backend_msg_log.FIRST .. ws_plugin_backend_msg_log.LAST LOOP
                    ...
                    EXECUTE IMMEDIATE 'UPDATE WS_PLUGIN_TB_BACKEND_MSG_LOG@' || db_link || ' SET MESSAGE_IDS = (SELECT MESSAGE_IDS FROM WS_PLUGIN_TB_BACKEND_MSG_LOG WHERE ID_PK = :p_1) WHERE ID_PK = :p_2'
                        USING ws_plugin_backend_msg_log(i).ID_PK,
                            ws_plugin_backend_msg_log(i).ID_PK;
                END LOOP;
    ...
    PROCEDURE migrate_message_acknw_prop(db_link IN VARCHAR2, migration IN T_MIGRATION_DETAILS) IS
        CURSOR c_message_acknw_prop IS
            SELECT ID_PK, PROPERTY_NAME, PROPERTY_VALUE, MESSAGE_ACK_ID_FK, CREATION_TIME, CREATED_BY
    ...
    BEGIN
        ...
        LOOP
            ...
            FOR i IN message_acknw_prop.FIRST .. message_acknw_prop.LAST LOOP
                EXECUTE IMMEDIATE 'INSERT INTO TB_MESSAGE_ACKNW_PROP@' || db_link || ' (ID_PK, PROPERTY_NAME, PROPERTY_VALUE, MESSAGE_ACK_ID_FK, CREATION_TIME, CREATED_BY) VALUES (:p_1, :p_2, :p_3, :p_4, :p_5, :p_6)'
                    USING message_acknw_prop(i).ID_PK,
                        message_acknw_prop(i).PROPERTY_NAME,
                        message_acknw_prop(i).PROPERTY_VALUE,
                        message_acknw_prop(i).MESSAGE_ACK_ID_FK,
...
--
-- Uncomment trailing line to execute the MIGRATE_ONGOING_MESSAGES_505.MIGRATE(..) procedure and to clean up the temporary tables
