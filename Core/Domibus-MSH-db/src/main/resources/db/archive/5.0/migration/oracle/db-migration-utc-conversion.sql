...
CREATE OR REPLACE PACKAGE BODY MIGRATE_42_TO_50_utc_conversion IS
    ...
    BEGIN
        ...
        UPDATE TB_CERTIFICATE
        SET NOT_VALID_BEFORE_DATE = SYS_EXTRACT_UTC(FROM_TZ(NOT_VALID_BEFORE_DATE, TIMEZONE))
        WHERE NOT_VALID_BEFORE_DATE IS NOT NULL;

        UPDATE TB_CERTIFICATE
        SET NOT_VALID_AFTER_DATE = SYS_EXTRACT_UTC(FROM_TZ(NOT_VALID_AFTER_DATE, TIMEZONE))
        ...
        UPDATE TB_MESSAGE_ACKNW
        SET CREATE_DATE = SYS_EXTRACT_UTC(FROM_TZ(CREATE_DATE, TIMEZONE))
        ...
        UPDATE TB_SEND_ATTEMPT
        SET START_DATE = SYS_EXTRACT_UTC(FROM_TZ(START_DATE, TIMEZONE))
        WHERE START_DATE IS NOT NULL;

        UPDATE TB_SEND_ATTEMPT
        SET END_DATE = SYS_EXTRACT_UTC(FROM_TZ(END_DATE, TIMEZONE))
