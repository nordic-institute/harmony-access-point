...
public class AlertContextConfiguration {
    ...
    public DefaultConfigurationManager defaultConfigurationManager(AlertType alertType) {
        return new DefaultConfigurationManager(alertType);
    }

    @Bean(autowireCandidate = false)
    @Scope(BeanDefinition.SCOPE_PROTOTYPE)
    public DefaultAlertConfigurationChangeListener defaultAlertConfigurationChangeListener(AlertType alertType, DomibusPropertyChangeNotifier domibusPropertyChangeNotifier) {
