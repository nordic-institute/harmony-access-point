...
public class JMSMessPgUXTest extends SeleniumTest {
    ...
    private String getSelector(HashMap<String, String> messInfo, String verb) throws JSONException {
        ...
        String selector = format("%s='%s' %s %s='%s'", key1, val1, verb, key2, val2);
        return selector;
    }
    ...
    /* EDELIVERY-8771 - JMS-39 - Delete All button */
    @Test(description = "JMS-39", groups = {"multiTenancy"})
    public void deleteAllButton() throws Exception {
        SoftAssert soft = new SoftAssert();
        ...
        if (page.grid().getRowsNo() == 0) {
            log.info("Checking if delete all button is enabled when no messages are present");
            soft.assertTrue(page.getDeleteAllButton().isPresent(), "Delete button is present");
        ...
        if (page.grid().getRowsNo() == 0) {
            log.info("Checking if delete all button is enabled when no messages are present after filtering");
            soft.assertFalse(page.getDeleteAllButton().isEnabled(), "Delete button is disabled after filtering to empty grid");
        }
        ...
        String messagesId = new JSONObject(rowInfo.get("Custom prop")).getString("messageId");
        String selector = format("messageId='%s'", messagesId);
        log.info("setting selector filter to: " + selector);
        page.filters().getJmsSelectorInput().fill(selector);
        page.filters().clickSearch();

        int initialNoOfMessages = rest.jms().getQueueMessages(q).length();
        int todel = page.grid().getRowsNo();

        log.info("deleting all LISTED messages");
        page.getDeleteAllButton().click();

        soft.assertEquals(page.grid().getRowsNo(), 0, "Grid is empty");

        int newNoOfMessages = rest.jms().getQueueMessages(q).length();

        soft.assertEquals(newNoOfMessages, initialNoOfMessages - todel, "Number of messages in queue is correct");
        ...
        newNoOfMessages = rest.jms().getQueueMessages(q).length();

        soft.assertEquals(newNoOfMessages, 0, "Number of messages in queue is 0");
