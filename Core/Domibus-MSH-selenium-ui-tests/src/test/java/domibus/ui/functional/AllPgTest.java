...
//				soft.assertNotEquals(searchData, new AlertPage(driver).filters().getAlertIdInput().getText(), "Grid has diff data for both domain -6");
    ...
    @Test(description = "ALLDOM-6", groups = {"multiTenancy", "singleTenancy"})
    public void allTextInputsAreTrimmed() throws Exception {
        SoftAssert soft = new SoftAssert();

        log.info("test all text inputs are trimmed on search");

        DomibusPage page = new DomibusPage(driver);

        for (PAGES pg : PAGES.values()) {
            page.getSidebar().goToPage(pg);

            try {
                FilterArea filters = page.getFilterArea();

                filters.fillAllInputs("  text  ");
                filters.clickSearch();

                filters.getAllInputsTexts().forEach(text -> {
                    soft.assertTrue(text.equalsIgnoreCase("text"), "Text is not trimmed for input in page " + pg.name());
                });
            } catch (Exception e) {
                log.info("Page " + pg.name() + " does not have any input text fields");
            }
        }
        soft.assertAll();
    }
    ...
    @Test(description = "ALLDOM-7", groups = {"multiTenancy", "singleTenancy"})
    public void filtersResetButton() throws Exception {
        SoftAssert soft = new SoftAssert();

        log.info("reset filters button works correctly");
        ...
        for (PAGES pg : PAGES.values()) {
            ...
            try {
                ...
                List<String> initialValues = filters.getAllInputsTexts();

                filters.fillAllInputs("  text  ");
                filters.clickReset();

                List<String> afterResetValues = filters.getAllInputsTexts();

                soft.assertTrue(initialValues.equals(afterResetValues), "Reset button does not work for page " + pg.name());
    ...
    @Test(description = "ALLDOM-8", groups = {"multiTenancy", "singleTenancy"})
    public void allExpiredCertificatesAreRed() throws Exception {
        SoftAssert soft = new SoftAssert();
        ...
        for (PAGES certPg : certPgs) {
            ...
            for (int i = 0; i < expirationDates.size(); i++) {
                Date currentDate = TestUtils.dateFromUIDateStr(expirationDates.get(i));
                if (currentDate.before(new Date())) {
                    tested = true;
                    log.info("Expired certificate found on page " + certPg.name() + " at row " + i);
                    soft.assertTrue(new DGrid(driver).getRowElement(i).getAttribute("class").contains("highlighted-row"),
                            "Expired certificate is highlighted in " + certPg.name());
                }
            }
        }
        if (!tested) {
            throw new SkipException("No expired certificates found");
        }
