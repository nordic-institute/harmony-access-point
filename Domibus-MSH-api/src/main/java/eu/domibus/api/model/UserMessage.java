...
@NamedQueries({
        ...
        @NamedQuery(name = "UserMessage.findTestMessage",
                query = "select um from UserMessage um where um.testMessage=true and um.sourceMessage=false and um.action.entityId=:ACTION_ID and um.partyInfo.to.toPartyId.value=:PARTY_ID order by um.entityId desc"),
})
@NamedNativeQueries({
        @NamedNativeQuery(
